...
void drawBag(Sav::Pouch pouch, int maxItem, int screenPos, bool background) {
	...
	if(background) {
		...
		for(unsigned i=0;i<save->pouches().size();i++) {
			...
			printTextMaxW(save->pouchName(save->language(), save->pouches()[i].first), boxButton.width-8, 1, 174, (104-(10*save->pouches().size()))+i*(20)+2, false, false);
		}
	}

	// TODO: Maybe switch this to the config language
	printText(save->pouchName(save->language(), pouch), 4, 0, false, true);

...
char16_t nextcharver(char16_t c) {
	...
	if(isKatakana)
		c -= 96;
	if((c >= 0x3041 && c <= 0x3061 && c % 2 && c != 0x3045) || (c == 0x3066 || c == 0x3068) ||
	   (c >= 0x306F && c <= 0x307C && (c % 3 != 2)) || (c >= 0x3083 && c <= 0x3087 && c % 2)) {
		c++;
	} else if(c == 0x3063) {
		c += 2;
	} else if((c >= 0x3042 && c <= 0x3062 && !(c % 2)) || (c == 0x3064 || c == 0x3065 || c == 0x3067 || c == 0x3069) ||
...
bool touching(const touchPosition &touch, int x, int y, int w, int h) {
	return (touch.px >= x + std::min(w, 0) && touch.px <= x + std::max(w, 0) && touch.py >= y + std::min(h, 0) &&
			touch.py <= y + std::max(h, 0));
}
...
void addToString(char16_t c) {
	...
	if(first == -1) {
		...
		} else if((c == 0x3131 && in == 0x3131) || (c == 0x3137 && in == 0x3137) || (c == 0x3142 && in == 0x3142) ||
				  (c == 0x3145 && in == 0x3145) || (c == 0x3148 && in == 0x3148)) {
			string[string.length() - 1]++;
		} else {
			string += c;
		}
...
void processInputABC(u16 held, unsigned maxLength) {
	...
	while(keysHeld() & (KEY_UP | KEY_DOWN | KEY_LEFT | KEY_RIGHT)) {
		...
		if(direction != -1) {
			...
			if(Config::getInt("keyboardDirections"))
				character =
					&(Config::getInt("keyboardGroupAmount") ? keysDPadABC4
															: keysDPadABC3)[direction + ((keysHeld() & KEY_L) ? 4 : 0)];
			else
				character = (keysHeld() & KEY_L)
					? &keysDPadABCSymbols[direction]
					: (Config::getInt("keyboardGroupAmount") ? &keysDPadABC4[direction] : &keysDPadABC3[direction]);
		...
		if(held & (KEY_A | KEY_B | KEY_X | KEY_Y)) {
			...
			if(direction != -1 && key != -1 && key < (int)keysDPadABC3[direction].size() && string.size() < maxLength) {
				std::u16string character;
				if(Config::getInt("keyboardDirections"))
					character = (Config::getInt("keyboardGroupAmount")
									 ? keysDPadABC4
									 : keysDPadABC3)[direction + ((keysHeld() & KEY_L) ? 4 : 0)];
				else
					character = (keysHeld() & KEY_L)
...
std::string Input::getLine(unsigned maxLength) {
	...
	while(1) {
		...
		if((loadedLayout >= 3 ? held : pressed) & KEY_TOUCH) {
			...
			else if(loadedLayout == 1)
				processTouchKana(touch, maxLength, keysABC);
			else if(loadedLayout == 2)
				processTouchKana(touch, maxLength, keysAIU);
			else if(loadedLayout == 3)
				processTouchQWERTY(touch, maxLength, keysQWERTY);
			else if(loadedLayout == 4)
				processTouchQWERTY(touch, maxLength, keysKor);
		...
		if(held & KEY_START || enter) {
			...
			if(loadedLayout < 3)
				Config::setInt("keyboardXPos", xPos);
			if(prevLayout != loadedLayout || prevXPos != xPos)
				Config::save();
			break;
		} else if(pressed & KEY_SELECT) {
			if(loadedLayout < 4)
				drawKeyboard(loadedLayout + 1);
			else
				drawKeyboard(0);
...
bool Input::getBool() {
	return getBool(i18n::localize(Config::getLang("lang"), "yes"), i18n::localize(Config::getLang("lang"), "no"));
}

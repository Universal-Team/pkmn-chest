...
void showDirectoryContents(const std::vector<DirEntry>& dirContents, int startRow) {
	...
	// Print path
	drawImageSegment(0, 0, 256, 17, listBg, 256, 0, 0, false);
	printTextMaxW(path, 250, 1, 4, 0, false);

	// Print directory listing
	for(int i=0;i < ENTRIES_PER_SCREEN; i++) {
		// Clear row
		drawImageSegment(10, i*16+16, 246, 16, listBg, 256, 10, i*16+16, false);
...
bool updateSlot1Text(int &cardWait, bool valid) {
	if(REG_SCFG_MC == 0x11) {
		...
		cardWait = 30;
		if(!noCardMessageSet) {
			drawImageSegment(10, ((tmSlot1Offset-tmScreenOffset)+1)*16+1, 200, 16, listBg, 256, 10, ((tmSlot1Offset-tmScreenOffset)+1)*16+1, false);
...
std::string topMenuSelect(void) {
	...
	// Clear screens
	drawImage(0, 0, 256, 192, boxBgTop, true);
	drawImage(0, 0, 256, 192, listBg, false);
	...
	while(1) {
		// Clear old cursors
		drawImageSegment(0, 17, 10, 175, listBg, 256, 0, 17, false);
		...
		if(held & KEY_UP || held & KEY_DOWN || held & KEY_LEFT || held & KEY_RIGHT || pressed & KEY_X) {
			...
			drawImage(0, 0, 256, 17, listBg, false);

...
void copyPalette(const Image &image, bool top, int paletteOffset = 0) {
	tonccpy((top ? BG_PALETTE : BG_PALETTE_SUB) + image.palOfs + paletteOffset,
			image.palette.data(),
			image.palette.size() * 2);
}
...
void drawImageDMA(int x, int y, const Image &image, bool top, bool layer, int paletteOffset, bool copyPal) {
	...
	for(int i = 0; i < image.height; i++) {
		dmaCopyHalfWords(
			0, image.bitmap.data() + (i * image.width), gfxPointer(top, layer) + ((y + i) * 256) + x, image.width);
	}
}

void drawImageScaled(int x,
					 int y,
					 float scaleX,
					 float scaleY,
					 const Image &image,
	...
	if(scaleX == 1 && scaleY == 1)
		drawImage(x, y, image, top, layer, paletteOffset);
	else {
		if(copyPal)
			copyPalette(image, top, paletteOffset);
		u8 *dst  = gfxPointer(top, layer);
...
void drawImageSegmentDMA(int x,
						 int y,
						 int w,
						 int h,
						 const Image &image,
	...
	for(int i = 0; i < h; i++) {
		dmaCopyHalfWords(0,
						 image.bitmap.data() + ((yOffset + i) * image.width) + xOffset,
						 gfxPointer(top, layer) + ((y + i) * 256) + x,
						 w);
	}
}

void drawImageSegmentScaled(int x,
							int y,
							int w,
							int h,
							float scaleX,
							...
							bool copyPal) {
	if(scaleX == 1 && scaleY == 1)
		return drawImageSegment(x, y, w, h, image, xOffset, yOffset, top, layer);
...
void drawRectangle(int x, int y, int w, int h, u8 color, bool top, bool layer) {
	drawRectangle(x, y, w, h, color, color, top, layer);
}
	...
	for(int i = 0; i < image.height; i++) {
		for(int j = 0; j < image.width; j++) {
			if(!(skipAlpha && image.palette[image.bitmap[(i * image.width) + j]] == 0)) {
				sprites(top)[id].gfx[((y + i) * spriteW) + (x + j)] =
					image.palette[image.bitmap[(i * image.width) + j] - image.palOfs];
			}
		}
	}
}

void fillSpriteImageScaled(
	int id, bool top, int spriteW, int x, int y, float scaleX, float scaleY, const Image &image) {
	if(scaleX == 1 && scaleY == 1)
		fillSpriteImage(id, top, spriteW, x, y, image);
	else {
		for(int i = 0; i < (image.height * scaleY); i++) {
			for(int j = 0; j < (image.width * scaleX); j++) {
				sprites(top)[id].gfx[(y + i) * spriteW + x + j] =
					image.palette[image.bitmap[(((int)(i / scaleY)) * image.width) + (j / scaleX)] - image.palOfs];
			}
		}
	}
}

void fillSpriteSegment(
	int id, bool top, int spriteW, int x, int y, int w, int h, const Image &image, int xOffset, int yOffset) {
	for(int i = 0; i < h; i++) {
		for(int j = 0; j < w; j++) {
			sprites(top)[id].gfx[((y + i) * spriteW) + x + j] =
				image.palette[image.bitmap[((i + yOffset) * image.width) + j + xOffset]];
		}
	}
}

void fillSpriteText(int id, bool top, const std::string &text, TextColor palette, int xPos, int yPos) {
	fillSpriteText(id, top, StringUtils::UTF8toUTF16(text), palette, xPos, yPos);
};
...
void prepareSprite(int id, bool top, int x, int y, int priority) {
	oamSet((top ? &oamMain : &oamSub), // Main/Sub display
		   id,                         // Oam entry to set
		   x,
		   y,                             // Position
		   priority,                      // Priority
...
void printText(const std::string &text, int xPos, int yPos, bool top, bool layer) {
	printTextTinted(StringUtils::UTF8toUTF16(text), TextColor::white, xPos, yPos, top, layer);
}
void printText(const std::u16string &text, int xPos, int yPos, bool top, bool layer) {
	printTextTinted(text, TextColor::white, xPos, yPos, top, layer);
}
void printTextCentered(const std::string &text, int xOffset, int yPos, bool top, bool layer) {
	printTextCenteredTinted(StringUtils::UTF8toUTF16(text), TextColor::white, xOffset, yPos, top, layer);
}
void printTextCentered(const std::u16string &text, int xOffset, int yPos, bool top, bool layer) {
	printTextCenteredTinted(text, TextColor::white, xOffset, yPos, top, layer);
}
void printTextCenteredTinted(const std::string &text, TextColor palette, int xOffset, int yPos, bool top, bool layer) {
	printTextCenteredTinted(StringUtils::UTF8toUTF16(text), palette, xOffset, yPos, top, layer);
}
void printTextCenteredTinted(std::u16string text, TextColor palette, int xOffset, int yPos, bool top, bool layer) {
	...
	while(text.find('\n') != text.npos) {
		printTextTinted(text.substr(0, text.find('\n')),
						palette,
						((256 - getTextWidth(text.substr(0, text.find('\n')))) / 2) + xOffset,
						yPos + (i++ * 16),
						top,
						layer);
...
void printTextTinted(const std::string &text, TextColor palette, int xPos, int yPos, bool top, bool layer) {
	printTextTinted(StringUtils::UTF8toUTF16(text), palette, xPos, yPos, top, layer);
}
...
void printTextMaxW(
	const std::string &text, float w, float scaleY, int xPos, int yPos, bool top, bool layer, float maxScale) {
	printTextTintedScaled(StringUtils::UTF8toUTF16(text),
						  std::min(std::min(1.0f, maxScale), w / getTextWidth(text)),
						  scaleY,
						  TextColor::white,
						  xPos,
						  yPos,
...
void printTextMaxW(
	const std::u16string &text, float w, float scaleY, int xPos, int yPos, bool top, bool layer, float maxScale) {
	printTextTintedScaled(text,
						  std::min(std::min(1.0f, maxScale), w / getTextWidth(text)),
						  scaleY,
						  TextColor::white,
						  xPos,
...
void printTextCenteredMaxW(
	const std::string &text, float w, float scaleY, int xOffset, int yPos, bool top, bool layer, float maxScale) {
	printTextCenteredTintedMaxW(
		StringUtils::UTF8toUTF16(text), w, scaleY, TextColor::white, xOffset, yPos, top, layer, maxScale);
}
void printTextCenteredMaxW(
	const std::u16string &text, float w, float scaleY, int xOffset, int yPos, bool top, bool layer, float maxScale) {
	printTextCenteredTintedMaxW(text, w, scaleY, TextColor::white, xOffset, yPos, top, layer, maxScale);
}
void printTextCenteredTintedMaxW(const std::string &text,
								 float w,
								 float scaleY,
								 TextColor palette,
								 int xOffset,
								 ...
								 float maxScale) {
	printTextCenteredTintedMaxW(
		StringUtils::UTF8toUTF16(text), w, scaleY, palette, xOffset, yPos, top, layer, maxScale);
}
void printTextCenteredTintedMaxW(std::u16string text,
								 float w,
								 float scaleY,
								 TextColor palette,
								 int xOffset,
	...
	while(text.find('\n') != text.npos) {
		printTextTintedScaled(text.substr(0, text.find('\n')),
							  ...
							  scaleY,
							  palette,
							  ((256 - getTextWidthMaxW(text.substr(0, text.find('\n')), w)) / 2) + xOffset,
							  yPos + (i++ * (16 * scaleY)),
							  top,
							  layer);
		text = text.substr(text.find('\n') + 1);
...
void printTextTintedMaxW(const std::string &text,
						 float w,
						 float scaleY,
						 TextColor palette,
						 int xPos,
...
void printTextTintedMaxW(const std::u16string &text,
						 float w,
						 float scaleY,
						 TextColor palette,
						 int xPos,
						 ...
						 bool layer,
						 float maxScale) {
	printTextTintedScaled(
		text, std::min(std::min(1.0f, maxScale), w / getTextWidth(text)), scaleY, palette, xPos, yPos, top, layer);
}

void printTextScaled(const std::string &text, float scaleX, float scaleY, int xPos, int yPos, bool top, bool layer) {
	printTextTintedScaled(StringUtils::UTF8toUTF16(text), scaleX, scaleY, TextColor::white, xPos, yPos, top, layer);
}
void printTextScaled(const std::u16string &text, float scaleX, float scaleY, int xPos, int yPos, bool top, bool layer) {
	printTextTintedScaled(text, scaleX, scaleY, TextColor::white, xPos, yPos, top, layer);
}
void printTextCenteredScaled(
	const std::string &text, float scaleX, float scaleY, int xOffset, int yPos, bool top, bool layer) {
	printTextCenteredTintedScaled(
		StringUtils::UTF8toUTF16(text), scaleX, scaleY, TextColor::white, xOffset, yPos, top, layer);
}
void printTextCenteredScaled(
	const std::u16string &text, float scaleX, float scaleY, int xOffset, int yPos, bool top, bool layer) {
	printTextCenteredTintedScaled(text, scaleX, scaleY, TextColor::white, xOffset, yPos, top, layer);
}
void printTextCenteredTintedScaled(const std::string &text,
								   float scaleX,
								   float scaleY,
								   TextColor palette,
								   int xOffset,
								   ...
								   bool top,
								   bool layer) {
	printTextCenteredTintedScaled(StringUtils::UTF8toUTF16(text), scaleX, scaleY, palette, xOffset, yPos, top, layer);
}
void printTextCenteredTintedScaled(
	std::u16string text, float scaleX, float scaleY, TextColor palette, int xOffset, int yPos, bool top, bool layer) {
	int i = 0;
	while(text.find('\n') != text.npos) {
		printTextTintedScaled(text.substr(0, text.find('\n')),
							  scaleX,
...
void printTextTintedScaled(
	const std::string &text, float scaleX, float scaleY, TextColor palette, int xPos, int yPos, bool top, bool layer) {
	printTextTintedScaled(StringUtils::UTF8toUTF16(text), scaleX, scaleY, palette, xPos, yPos, top, layer);
}

void printTextTintedScaled(const std::u16string &text,
						   float scaleX,
						   float scaleY,
						   TextColor palette,
						   int xPos,
...
int getTextWidthMaxW(const std::string &text, int w) {
	return std::min(w, getTextWidth(StringUtils::UTF8toUTF16(text)));
}
...
int getTextWidthScaled(const std::string &text, float scale) {
	return getTextWidth(StringUtils::UTF8toUTF16(text)) * scale;
}
